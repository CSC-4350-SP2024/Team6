  ...
  return (
    <ScrollView>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#1044a9', paddingHorizontal: 10 }}>
        <Text style={[{ color: 'white' }, { paddingTop: 20 },  { fontSize: 17 }, { backgroundColor: '#1044a9' }]}>   Welcome, {userName || ''} </Text>
        {}
        <Image
      ...
      <CalendarStrip
        ...
        calendarHeaderStyle={{ color: 'white', fontSize: 20, paddingBottom: '2%' }}
      ...
      <View style={{ backgroundColor: '#1044a9' }}>
        {classesForToday.map((element, index) => <Text key={index} style={[{ color: '#1044a9' }, { padding: 10 }, { fontSize: 15 }, { backgroundColor: '#D1DFFB' }, { textAlign: 'center' }]}> {element.classname}: {element.sectioncrn} {toAmPm(element.starttime[0])} - {toAmPm(element.endtime[0])} </Text>)}
      </View>
